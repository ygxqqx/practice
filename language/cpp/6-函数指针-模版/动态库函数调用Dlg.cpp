...
C动态库函数调用Dlg::C动态库函数调用Dlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(C动态库函数调用Dlg::IDD, pParent)
{
...
BEGIN_MESSAGE_MAP(C动态库函数调用Dlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON1, &C动态库函数调用Dlg::OnBnClickedButton1)
END_MESSAGE_MAP()
